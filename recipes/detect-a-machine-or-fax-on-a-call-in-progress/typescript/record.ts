/**
 * The recorder verify.py drives. It swaps the client's fetch, tries every
 * helper without a status URL, then calls all four properly, and prints what it
 * captured as JSON on stdout.
 *
 * The expected values live in verify.py, which holds this output and the Python
 * surface's requests to the same one set.
 */
import { client, machine, fax, digits, stop } from "./index.js";

type Captured = { method: string; path: string; body: unknown };
const captured: Captured[] = [];

const recorder: typeof fetch = async (input, init) => {
  const url = new URL(String(input));
  captured.push({
    method: init?.method ?? "GET",
    path: url.pathname,
    body: typeof init?.body === "string" ? JSON.parse(init.body) : null,
  });
  return new Response("{}", { status: 200, headers: { "content-type": "application/json" } });
};

// the seam the Python verifier uses, where it swaps client._http
(client.calling as unknown as { _http: { _fetch: typeof fetch } })._http._fetch = recorder;

const [callId = "", statusUrl = ""] = process.argv.slice(2);

// a detect with no status_url never reaches the wire, missing or empty
const refused: string[] = [];
for (const bad of [undefined, ""]) {
  for (const helper of [machine, fax, digits]) {
    try {
      await helper(callId, bad);
      refused.push("SENT");
    } catch (error) {
      refused.push(String((error as Error).message));
    }
  }
}

await machine(callId, statusUrl);
await fax(callId, statusUrl);
await digits(callId, statusUrl);
await stop(callId);

console.log(JSON.stringify({ refused, captured }));
